...
# Load FCS files ----------------------------------------------------------
myfiles <- "raw_gating_samples/water_matrix"
...
cyto_save(fs_comp_clean,
          save_as = "MatrixStudy-Samples")

# Samples as GatingSet
gs <- cyto_setup("MatrixStudy-Samples") #IF want to create new gating template add gatingTemplate = "GACLiveDead-gatingTemplate.csv"
...
# GAC Study Live/Dead Analysis --------------------------------------------
# Extract the pData from the GatingSet
pdata <- pData(gs_gated)
colnames(pdata)
# Subset pData where dye is "both" and sample contains "replicate"
#subset_indices <- which(pdata$reactor %in% c("s3") & pdata$replicate %in% c("triplicate"))
subset_indices <- which(pdata$dye %in% c("both") & pdata$replicate %in% c("sample"))
...
autoplot(gs_subset[[3]], x = "SYBR Green I-A", y= "Propidium Iodide-A", bins = 256)

# In case need to edit gate
cyto_gatingTemplate_active() #to check which gate file is being active

gs_sgi <- gs_comp_clean_trans["A02 20240613_di1_both.fcs"]
cyto_gate_edit(gs_sgi, 
               parent = "root",
               alias = "Intact Cells",
...
cyto_plot_gating_tree(gs_subset[[1]],
                      stat = "freq")


# get population stats for downstream analysis
ps <- gs_pop_get_count_with_meta(gs_subset)
...
#subset_indices <- which(pdata$reactor %in% c("s3") & pdata$replicate %in% c("triplicate"))
subset_indices <- which(pdata$dye %in% c("both") & pdata$replicate %in% c("sample"))
#subset_indices <- which(pdata$matrix %in% c("1xPBS") & pdata$replicate %in% c("sample"))
# Subset the GatingSet using the identified indices
gs_subset <- gs_gated[subset_indices]
pData(gs_subset)
...
# Save png image of gating scheme after plotting
plot_name <- paste0("Gating-plots/",pData(gs_subset)$reactor[1], "_", pData(gs_subset)$replicate[1], "_Matrix-Gating-Scheme.png")
cyto_plot_save(plot_name,
               width = 10, #1349x1228 = 1349/300, 1228/300 = width=4.50, height=4.09
               height = 10,
               res = 300 #high resolution for publication
